...
const ModelSelector = () => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        className={`flex items-center gap-2 bg-transparent hover:bg-[#3f3f3f] text-white px-3 py-2 rounded-xl border border-transparent hover:border-gray-600 cursor-pointer transition-all ${models.length === 0 ? "hidden" : "flex"
          }`}
                    ...
                    {model.image && (
                      <span title="Image support" className="flex items-center">
                        <svg
                          aria-hidden="true"
                          viewBox="0 0 24 24"
                          className={`h-3.5 w-3.5 ${isSelected ? "fill-emerald-300" : "fill-gray-500"
                            }`}
                        >
