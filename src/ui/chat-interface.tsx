...
const ChatInterface = ({ id }: { id: string }) => {
	...
	return (
		...
		>
			...
			{/* Minimap */}
			{messages.length > 0 && (
				<div className="fixed right-2 top-1/2 -translate-y-1/2 z-10 hidden md:flex flex-col gap-2 bg-neutral-800/80 backdrop-blur-sm p-2 rounded-lg max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-600 scrollbar-track-transparent">
					{messages
						...
						.map(({ index }) => (
							<button
								...
								className="w-3 h-3 rounded-full bg-cyan-500 hover:bg-cyan-400 hover:scale-125 transition-all duration-200 cursor-pointer shadow-sm"
								title={messages[index].content.replace(/\n/g, " ").slice(0, 50)}
							/>
						))}
				</div>
			)}
